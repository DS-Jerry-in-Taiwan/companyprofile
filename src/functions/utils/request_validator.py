...
def validate_request(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if mode not in ('GENERATE', 'OPTIMIZE'):
        raise ValidationError('mode must be GENERATE or OPTIMIZE', details=[
            {'field': 'mode', 'reason': 'unsupported mode'}
        ])
    ...
    if mode == 'OPTIMIZE':
        ...
        if not brief or not isinstance(brief, str):
            raise ValidationError('brief is required for OPTIMIZE mode', details=[
                {'field': 'brief', 'reason': 'required in OPTIMIZE mode'}
            ])

    word_limit = data.get('word_limit')
    if word_limit is not None:
        if not isinstance(word_limit, int) or word_limit < 50 or word_limit > 2000:
            raise ValidationError('word_limit must be an integer between 50 and 2000', details=[
                {'field': 'word_limit', 'reason': 'out of range'}
            ])

    optimization_mode = data.get('optimization_mode', 'STANDARD')
    if optimization_mode not in ('STANDARD', 'CONCISE', 'DETAILED'):
        raise ValidationError('optimization_mode must be STANDARD, CONCISE or DETAILED', details=[
            {'field': 'optimization_mode', 'reason': 'invalid enum value'}
        ])
